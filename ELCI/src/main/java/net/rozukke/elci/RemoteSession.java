...
public class RemoteSession {
	...
	public void tick() throws Exception {
		...
		while ((message = inQueue.poll()) != null) {
		    // Perform decryption on message before handling
			plugin.getLogger().info("Received encrypted message:");
			plugin.getLogger().info(message);

			// Read enc.key
			byte[] enc_key = Objects.requireNonNull(this.getClass().getResourceAsStream("/enc.key")).readAllBytes();

			// Read mac.key
			byte[] mac_key = Objects.requireNonNull(this.getClass().getResourceAsStream("/mac.key")).readAllBytes();

			// Separate first 32 bytes as mac_code
			byte[] decodedBytes = Base64.getDecoder().decode(message);
			byte[] received_mac_tag = Arrays.copyOfRange(decodedBytes, 0, 32);
			byte[] received_cipherdata = Arrays.copyOfRange(decodedBytes, 32, decodedBytes.length);

			// Verify MAC Tag
			try {
				AESUtils.verify_hmac_sha_256_tag(received_mac_tag, received_cipherdata, mac_key);
			} catch (Exception e) {
				plugin.getLogger().info("Invalid signature.");
			}

			// Perform decryption
			String resultString = AESUtils.aes_256_cbc_decrypt(received_cipherdata, enc_key);

			// Print to server console
			plugin.getLogger().info("Decrypted command:");
			plugin.getLogger().info(resultString);

			// Handle command
			handleLine(resultString);
			processedCount++;
...
//		System.out.println(line);
	...
	protected void handleCommand(String c, String[] args) throws Exception {
	...
	public void send(Object a) throws Exception {
		send(a.toString());
	}
	...
	public void send(String a) throws Exception {
		...
		synchronized(outQueue) {
			// Encrypt a before sending back to Python
			plugin.getLogger().info("Sending the following response:");
			...
			// Encrypt data
			byte[] cipherData = AESUtils.aes_256_cbc_encrypt(a.getBytes(), enc_key);

			// Create MAC Tag
			byte[] macTag = AESUtils.create_hmac_sha_256_tag(cipherData, mac_key);

			// Concatenate
			byte[] outboundData = Arrays.copyOf(macTag, macTag.length + cipherData.length);
			System.arraycopy(cipherData, 0, outboundData, macTag.length, cipherData.length);
			String outboundDataB64Encoded = Base64.getEncoder().encodeToString(outboundData);
			outboundDataB64Encoded = outboundDataB64Encoded + "\n";

			// Log encoded response to console
			plugin.getLogger().info(outboundDataB64Encoded);

			// Send string back to Python
			outQueue.add(outboundDataB64Encoded);
		}
	}
